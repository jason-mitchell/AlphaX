...
void ServiceMeter(void){
	...
	if(UPDATE_PENDING == 1){
		...
		if(GetWordSize() == AUDIO_24_BITS){
			...
			// For the most part this gives accurate dB levels but is highly susceptible to peaks for some reason
			// it needs to be damped
			// The other concern is that its eating CPU cycles... need to move this out of interrupt...

			// TODO: Develop algorithm using desktop PC viz C++ MFC application so that we can easily play
			// and possibly borrow code from Audacity or other open-source projects
			/*LEFT_CHANNEL = LEFT_PCM;
			sum = LEFT_CHANNEL;
			sum = fabs(sum);
			db = (sum > 0) ? 20 * log10(sum / REFERENCE) : -144.4943;
